"use strict";Object.defineProperty(exports, "__esModule", {value: true}); function _interopRequireWildcard(obj) { if (obj && obj.__esModule) { return obj; } else { var newObj = {}; if (obj != null) { for (var key in obj) { if (Object.prototype.hasOwnProperty.call(obj, key)) { newObj[key] = obj[key]; } } } newObj.default = obj; return newObj; } } function _interopRequireDefault(obj) { return obj && obj.__esModule ? obj : { default: obj }; } function _nullishCoalesce(lhs, rhsFn) { if (lhs != null) { return lhs; } else { return rhsFn(); } } function _optionalChain(ops) { let lastAccessLHS = undefined; let value = ops[0]; let i = 1; while (i < ops.length) { const op = ops[i]; const fn = ops[i + 1]; i += 2; if ((op === 'optionalAccess' || op === 'optionalCall') && value == null) { return undefined; } if (op === 'access' || op === 'optionalAccess') { lastAccessLHS = value; value = fn(value); } else if (op === 'call' || op === 'optionalCall') { value = fn((...args) => value.call(lastAccessLHS, ...args)); lastAccessLHS = undefined; } } return value; }

var _chunkGPQD2JMXcjs = require('./chunk-GPQD2JMX.cjs');

// src/index.ts
var _unplugin = require('unplugin');
var _getportplease = require('get-port-please');

// src/core/constants.ts
var PLUGIN_NAME = "unplugin-turbo-console";
var NUXT_CONFIG_KEY = "turboConsole";

// src/core/server/index.ts
var _http = require('http');
var _process = require('process');
var _pathe = require('pathe');
var _h3 = require('h3');
var _servestatic = require('serve-static'); var _servestatic2 = _interopRequireDefault(_servestatic);
var _launcheditor = require('launch-editor'); var _launcheditor2 = _interopRequireDefault(_launcheditor);

// src/core/dir.ts
var _url = require('url');

var DIR_DIST = typeof __dirname !== "undefined" ? __dirname : _pathe.dirname.call(void 0, _url.fileURLToPath.call(void 0, _chunkGPQD2JMXcjs.importMetaUrl));
var DIR_CLIENT = _pathe.resolve.call(void 0, DIR_DIST, "./client/index.html");
var DIR_INTRO = _pathe.resolve.call(void 0, DIR_DIST, "./client/intro.html");

// src/core/server/index.ts
async function startServer(port = 3070) {
  try {
    await fetch(`http://localhost:${port}/health`);
  } catch (error) {
    const app = _h3.createApp.call(void 0, );
    app.use("/client", _h3.fromNodeMiddleware.call(void 0, _servestatic2.default.call(void 0, DIR_CLIENT)));
    app.use("/intro", _h3.fromNodeMiddleware.call(void 0, _servestatic2.default.call(void 0, DIR_INTRO)));
    app.use("/health", _h3.eventHandler.call(void 0, () => {
      return {
        message: "ok"
      };
    }));
    app.use("/__open-in-editor", _h3.eventHandler.call(void 0, async (event) => {
      try {
        const { file } = _h3.getQuery.call(void 0, event);
        _launcheditor2.default.call(void 0, _pathe.resolve.call(void 0, _process.cwd.call(void 0, ), file));
        return {
          message: "ok"
        };
      } catch (error2) {
        return {
          error: String(error2)
        };
      }
    }));
    _http.createServer.call(void 0, _h3.toNodeListener.call(void 0, app)).listen(port);
  }
}

// src/core/transform/vite.ts
var _magicstring = require('magic-string'); var _magicstring2 = _interopRequireDefault(_magicstring);
var _astkit = require('ast-kit');
var _sourcemapjs = require('source-map-js');

// src/core/transform/common.ts

var _buffer = require('buffer');


// src/core/utils.ts

var _pluginutils = require('@rollup/pluginutils');
var filter = _pluginutils.createFilter.call(void 0, 
  [/\.vue$/, /\.vue\?vue/, /\.vue\?v=/, /\.ts$/, /\.tsx$/, /\.js$/, /\.jsx$/, /\.svelte$/],
  [/[\\/]node_modules[\\/]/, /[\\/]\.git[\\/]/, /[\\/]\.nuxt[\\/]/]
);
var commonStyle = "padding:2px 5px; border-radius:3px 0 0 3px;margin-bottom:5px;";
var consoleStyles = {
  ".js": `${commonStyle}color: #111827; background: #F7DF1E`,
  ".jsx": `${commonStyle}color: #111827; background: #F7DF1E`,
  ".ts": `${commonStyle}color: #fff; background: #3178C6`,
  ".tsx": `${commonStyle}color: #fff; background: #3178C6`,
  ".vue": `${commonStyle}color: #fff; background: #4FC08D`,
  ".svelte": `${commonStyle}color: #fff; background: #FF3E00`,
  ".astro": `${commonStyle}color: #fff; background: #FF5D01`,
  "default": `${commonStyle}color: #111827; background: #F7DF1E`
};
function getConsoleStyle(fileType) {
  return _nullishCoalesce(consoleStyles[fileType], () => ( consoleStyles.default));
}
var launchEditorStyle = "background: #00DC8250;padding:2px 5px;border-radius:0 3px 3px 0;margin-bottom:5px";
var getEnforce = {
  rollup: "post",
  vite: "post",
  webpack: "pre",
  esbuild: "post",
  rspack: "pre"
};
function printInfo(port) {
  console.log("\x1B[32m%s\x1B[0m\x1B[1m%s\x1B[0m\x1B[36m%s\x1B[0m", "  \u279C", `  TurboConsole:`, ` http://localhost:${port}/intro`);
}
function getFileNameWithoutExtension(fileName) {
  if (!fileName)
    return "";
  return fileName.replace(/\.[^/.]+$/, "");
}
function getExtendedPath(filePath, extendedPathFileNames) {
  const arr = filePath.split(_pathe.sep);
  let basename = arr.pop() || "";
  const basenameWithoutExt = getFileNameWithoutExtension(basename).toLowerCase();
  if (extendedPathFileNames && _optionalChain([extendedPathFileNames, 'optionalAccess', _ => _.length]) > 0) {
    let isEnd = false;
    if (extendedPathFileNames.some((name) => basenameWithoutExt === name.toLowerCase())) {
      while (!isEnd) {
        const fileName = arr.pop();
        basename = `${fileName}/${basename}`;
        if (extendedPathFileNames.every((name) => name.toLowerCase() !== _optionalChain([fileName, 'optionalAccess', _2 => _2.toLowerCase, 'call', _3 => _3()])))
          isEnd = true;
      }
    }
  }
  return basename;
}

// src/core/transform/common.ts
function genConsoleString(genContext) {
  const { options, originalColumn, originalLine, argType, id } = genContext;
  let { argsName } = genContext;
  const { prefix, suffix, disableLaunchEditor, port, disableHighlight, extendedPathFileNames } = options;
  const _prefix = prefix ? `${prefix}\\n` : "";
  const _suffix = suffix ? `\\n${suffix}` : "";
  const urlObject = new URL(id, "file://");
  const filePath = urlObject.pathname;
  const fileName = getExtendedPath(filePath, extendedPathFileNames);
  const fileType = _pathe.extname.call(void 0, filePath);
  try {
    argsName = JSON.parse(`"${argsName}"`);
  } catch (error) {
  }
  if (_optionalChain([argsName, 'optionalAccess', _4 => _4.length]) > 30)
    argsName = `${argsName.slice(0, 30)}...`;
  const lineInfo = `%c\u{1F680} ${fileName}\xB7${originalLine}${["StringLiteral", "NumericLiteral"].includes(argType) ? "" : ` ~ ${argsName}`}`;
  const codePosition = `${_pathe.relative.call(void 0, _process.cwd.call(void 0, ), filePath)}:${originalLine}:${(originalColumn || 0) + 1}`;
  const launchEditorString = `%c\u{1F526} http://localhost:${port}/client#${_buffer.Buffer.from(codePosition, "utf-8").toString("base64")}`;
  let consoleString = "";
  if (!disableHighlight && !disableHighlight) {
    consoleString = _prefix ? `"${_prefix}${lineInfo}${launchEditorString}","${getConsoleStyle(fileType)}","${launchEditorStyle}","\\n",` : `"${lineInfo}${launchEditorString}","${getConsoleStyle(fileType)}","${launchEditorStyle}","\\n",`;
  }
  if (disableHighlight && !disableLaunchEditor) {
    consoleString = _prefix ? `"${_prefix}${launchEditorString}","${launchEditorStyle}","\\n",` : `"${launchEditorString}","${launchEditorStyle}","\\n",`;
  }
  if (!disableHighlight && disableLaunchEditor) {
    consoleString = _prefix ? `"${_prefix}${lineInfo}","${getConsoleStyle(fileType)}","\\n",` : `"${lineInfo}","${getConsoleStyle(fileType)}","\\n",`;
  }
  if (disableHighlight && disableLaunchEditor) {
    consoleString = _prefix ? `"${_prefix}",` : "";
  }
  return {
    consoleString,
    _suffix
  };
}
function isConsoleExpression(node) {
  return node.type === "CallExpression" && node.callee.type === "MemberExpression" && node.callee.object.type === "Identifier" && node.callee.object.name === "console" && node.callee.property.type === "Identifier" && _optionalChain([node, 'access', _5 => _5.arguments, 'optionalAccess', _6 => _6.length]) > 0;
}

// src/core/transform/vite.ts
function viteTransform(context) {
  const { code, id, pluginContext, options } = context;
  const magicString = new (0, _magicstring2.default)(code);
  const program = _astkit.babelParse.call(void 0, code, _astkit.getLang.call(void 0, id), {
    sourceFilename: id
  });
  _astkit.walkAST.call(void 0, program, {
    enter(node) {
      if (isConsoleExpression(node)) {
        const expressionStart = node.start;
        const expressionEnd = node.end;
        const originalExpression = magicString.slice(expressionStart, expressionEnd);
        if (originalExpression.includes("%c"))
          return false;
        const { line, column } = node.loc.start;
        const args = node.arguments;
        const sourceMap = pluginContext.getCombinedSourcemap();
        const consumer = new (0, _sourcemapjs.SourceMapConsumer)(sourceMap);
        const { line: originalLine, column: originalColumn } = consumer.originalPositionFor({
          line,
          column
        });
        const argsStart = args[0].start;
        const argsEnd = args[args.length - 1].end;
        const argType = args[0].type;
        const argsName = magicString.slice(argsStart, argsEnd).toString().replace(/`/g, "").replace(/\n/g, "").replace(/"/g, "");
        const { consoleString, _suffix } = genConsoleString({
          options,
          originalLine,
          originalColumn,
          argType,
          argsName,
          id
        });
        consoleString && magicString.appendLeft(argsStart, consoleString);
        _suffix && magicString.appendRight(argsEnd, `,"${_suffix}"`);
      }
    }
  });
  return {
    code: magicString.toString(),
    map: magicString.generateMap({ source: id })
  };
}

// src/core/transform/webpack.ts


var vuePatterns = [/\.vue$/, /\.vue\?vue/, /\.vue\?v=/];
async function webpackTransform(context) {
  const { id, code, options } = context;
  let scriptString = code;
  let scriptLang = _astkit.getLang.call(void 0, context.id);
  let vueSfcLocStart = {
    line: 0,
    column: 0,
    offset: 0
  };
  const magicString = new (0, _magicstring2.default)(code);
  if (vuePatterns.some((pattern) => pattern.test(id))) {
    const { parse } = await Promise.resolve().then(() => _interopRequireWildcard(require("vue/compiler-sfc")));
    const { descriptor, errors } = parse(code, {
      filename: id
    });
    if (errors.length === 0) {
      if (descriptor.script) {
        scriptString = descriptor.script.content;
        scriptLang = descriptor.script.lang || "";
        vueSfcLocStart = descriptor.script.loc.start;
      } else if (descriptor.scriptSetup) {
        scriptString = descriptor.scriptSetup.content;
        scriptLang = descriptor.scriptSetup.lang || "";
        vueSfcLocStart = descriptor.scriptSetup.loc.start;
      }
      vueSfcLocStart.line--;
    }
  }
  const program = _astkit.babelParse.call(void 0, scriptString, scriptLang, {
    sourceFilename: id
  });
  _astkit.walkAST.call(void 0, program, {
    enter(node) {
      if (isConsoleExpression(node)) {
        const expressionStart = node.start;
        const expressionEnd = node.end;
        const originalExpression = magicString.slice(expressionStart, expressionEnd);
        if (originalExpression.includes("%c"))
          return false;
        const { line, column } = node.loc.start;
        const args = node.arguments;
        const argsStart = args[0].start + vueSfcLocStart.offset;
        const argsEnd = args[args.length - 1].end + vueSfcLocStart.offset;
        const argType = args[0].type;
        const argsName = magicString.slice(argsStart, argsEnd).toString().replace(/`/g, "").replace(/\n/g, "").replace(/"/g, "");
        const originalLine = line + vueSfcLocStart.line;
        const originalColumn = column;
        const { consoleString, _suffix } = genConsoleString({
          options,
          originalLine,
          originalColumn,
          argType,
          argsName,
          id
        });
        consoleString && magicString.appendLeft(argsStart, consoleString);
        _suffix && magicString.appendRight(argsEnd, `,"${_suffix}"`);
      }
    }
  });
  return {
    code: magicString.toString(),
    map: magicString.generateMap({ source: id })
  };
}

// src/core/transform/transformer.ts
function transformer(context) {
  const { meta } = context;
  if (["webpack", "rspack"].includes(meta.framework))
    return webpackTransform(context);
  else
    return viteTransform(context);
}

// src/core/options.ts

var DETAULT_OPTIONS = {
  prefix: "",
  suffix: "",
  disableLaunchEditor: false,
  disableHighlight: false,
  extendedPathFileNames: [],
  port: 3070
};
var BUILD_OPTIONS = {
  disableLaunchEditor: true,
  disableHighlight: true
};
function resolveOptions(options) {
  let resolved = {
    ...DETAULT_OPTIONS,
    ...options
  };
  if (_process.env.NODE_ENV === "production") {
    resolved = {
      ...resolved,
      ...BUILD_OPTIONS
    };
  }
  return resolved;
}

// src/index.ts
var unpluginFactory = (rawOptions = {}, meta) => {
  const options = resolveOptions(rawOptions);
  async function detectPort() {
    const isAvailable = await _getportplease.checkPort.call(void 0, options.port);
    if (!isAvailable)
      options.port = await _getportplease.getRandomPort.call(void 0, );
  }
  return {
    name: PLUGIN_NAME,
    enforce: getEnforce[meta.framework] || "post",
    transformInclude(id) {
      return filter(id);
    },
    async transform(code, id) {
      const context = {
        options,
        pluginContext: this,
        code,
        id,
        meta
      };
      const result = await transformer(context);
      return result;
    },
    vite: {
      async configureServer(server) {
        if (options.disableLaunchEditor)
          return;
        await detectPort();
        const _print = server.printUrls;
        server.printUrls = () => {
          _print();
          printInfo(options.port);
        };
        startServer(options.port);
      }
    },
    async webpack(compiler) {
      if (options.disableLaunchEditor)
        return;
      await detectPort();
      if (compiler.options.mode === "development") {
        compiler.hooks.done.tap(PLUGIN_NAME, (state) => {
          if (state.hasErrors())
            return;
          printInfo(options.port);
          startServer(options.port);
        });
      }
    },
    async rspack(compiler) {
      if (options.disableLaunchEditor)
        return;
      await detectPort();
      if (compiler.options.mode === "development") {
        compiler.hooks.done.tap(PLUGIN_NAME, (state) => {
          if (state.hasErrors())
            return;
          printInfo(options.port);
          startServer(options.port);
        });
      }
    }
  };
};
var unplugin = /* @__PURE__ */ _unplugin.createUnplugin.call(void 0, unpluginFactory);
var src_default = unplugin;







exports.PLUGIN_NAME = PLUGIN_NAME; exports.NUXT_CONFIG_KEY = NUXT_CONFIG_KEY; exports.unpluginFactory = unpluginFactory; exports.unplugin = unplugin; exports.src_default = src_default;
