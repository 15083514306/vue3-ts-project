  ...
  actions: {
    ...
    async user_info() {
      ...
      if (res.code === 200) {
        ...
        // 统一小写处理
        const resRoutes: string[] = res.data.routes.map((name) => name.toLowerCase());
        ...
        const userRoutes: RouteRecordRaw[] = filterAsyncRoutes(cloneDeep(asyncRoutes), resRoutes);
...
// 过滤当前用户名下拥有哪些异步路由
function filterAsyncRoutes(allAsyncRoutes: RouteRecordRaw[], resRoutes: string[]): RouteRecordRaw[] {
