...
// 根据ID查询所有角色
export const reqAllRoleById = (userId: number) => request.get<any, AllRoleResponseData>(API.ALL_ROLE_URL + userId);

// 设置用户的角色
export const reqSetUserRole = (data: SetRoleData) => request.post<any, BasicResponse>(API.SET_ROLE_URL, data);

// 删除用户
export const reqRemoveUser = (userId: number) => request.delete<any, BasicResponse>(API.DELETE_USER_URL + userId);
