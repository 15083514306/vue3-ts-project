...
export const reqSetPermission = (roleId: number, permissionId: number[]) =>
  request.post<any, BasicResponse>(API.SET_PERMISSION_URL + `roleId=${roleId}&permissionId=${permissionId}`);

export const reqRemoveRole = (roleId: number) => request.delete<any, BasicResponse>(API.REMOVE_ROLE_URL + roleId);
