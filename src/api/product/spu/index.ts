...
import { SaleAttrValueResData, SpuImgListResData, SpuInfo, SpuInfoPageNumResData } from "@/api/product/spu/type.ts";
...
// 获取SPU信息
export const reqGetSpuInfo = (page: number, limit: number, category3Id: number | string) =>
  request.get<any, SpuInfoPageNumResData>(`${API.GET_SPU_INFO}/${page}/${limit}?category3Id=${category3Id}`);

// 获取销售属性相关信息
export const reqGetSaleAttrInfo = () => request.get<any, BasicResponse>(API.GET_SALE_ATTR_INFO);

// 获取所有的品牌
export const reqGetAllTradeMark = () => request.get<any, AllTradeMarListResData>(API.GET_ALL_TRADEMARK_URL);
...
export const reqGetSaleAttrListById = (spuId: number) =>
  request.get<any, SaleAttrValueResData>(`${API.GET_SALE_ATTR_LIST_BY_ID}/${spuId}`);

// 添加或修改SPU信息
export const reqAddOrUpdateSpuInfo = (spuInfo: SpuInfo): Promise<BasicResponse> => {
  if (spuInfo.id) {
    // 修改
