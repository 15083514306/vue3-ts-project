...
import { SkuDetailByIdResData, SkuInfo, SkuListByIdResData, SkuListByPageResData } from "@/api/product/sku/type.ts";
...
export const reqGetSkuListByPage = (page: number, limit: number) =>
  request.get<any, SkuListByPageResData>(`${API.GET_SKU_INFO_BY_PAGE}/${page}/${limit}`);
...
// 添加SKU信息
export const reqAddSkuInfo = (skuInfo: SkuInfo) => request.post<any, BasicResponse>(API.ADD_SKU_INFO, skuInfo);

// 删除SKU信息
export const reqDeleteSkuInfo = (skuId: number) => request.delete<any, BasicResponse>(API.DELETE_SKU_INFO + skuId);

// 上架
export const reqOnSale = (skuId: number) => request.get<any, BasicResponse>(API.ON_SALE + skuId);

// 下架
export const reqCancelSale = (skuId: number) => request.get<any, BasicResponse>(API.CANCEL_SALE + skuId);
